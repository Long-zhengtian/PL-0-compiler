...
public:
    GandSAnalyzer(){
        ...
        TCGMachine.init(); stable.init();
    };
    explicit GandSAnalyzer(vector<Word> _input){
        now=0; level=0; result.clear(); TreeRoot=nullptr; input.clear();
        input=move(_input); TCGMachine.init(); stable.init();
    };
    bool analyze();
    bool PROGRAM();//主程序
...
private:
    ...
    TargetCodeGenerator TCGMachine;//建立目标代码产生器
    SymbolTable stable;//建立符号表
